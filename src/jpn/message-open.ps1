...
"@
        ...
        for ($batchStart = $minEventId; $batchStart -le $maxEventId; $batchStart += $batchSize) {
            ...
            $fileName = "msg-${pesRegion}_${custId}_messageOpen_${todayDate}_${cdmsInstance}-${todayTime}-batch${batchNum}"
            $outputFile = Join-Path $backfillDir "${fileName}-raw.tsv"
            $sproc = "EXEC $custDbName.dbo.p_pes_backfill_open_get @min_event_id=$batchStart, @max_event_id=$batchEnd, @region='$pesRegion'"
