...
"@
        ...
        for ($batchStart = $minEventId; $batchStart -le $maxEventId; $batchStart += $batchSize) {
            ...
            $todayDate = Get-Date -Format "yyyy-MM-dd"
            $todayTime = Get-Date -Format "HHmmss"    
            $fileName = "msg-${pesRegion}_${custId}_messageWebEvent_${todayDate}_${cdmsInstance}-${todayTime}-batch${batchNum}"
            $outputFile = Join-Path $backfillDir "${fileName}-raw.tsv"
            $sproc = "EXEC $custDbName.dbo.p_pes_backfill_webevent_get @min_event_id=$batchStart, @max_event_id=$batchEnd, @region='$pesRegion'"
    ...
    foreach ($file in $files) {
        ...
        $fileName = $file.Name
        $uploadResult = aws s3 cp $filePath "s3://es-loader-an1-prod03/esl-service/incoming/$fileName" --profile "apac_backfill"
